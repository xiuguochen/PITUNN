...
% created by：YSL
% date：2023.6
function  [N, C, S, M_basis] = compressed_sensing(Y, cos_delta_2, cos_delta_1Minus2,...
    sin_delta_1Minus2, cos_delta_1Plus2, sin_delta_1Plus2, L, DCT, beta, sample_points, basis)
% 用于通道型Stokes偏振仪的计算
...
%% 压缩感知方法求解stokes参量
% 构造三角函数矩阵
M_tri_N = -2*diag(cos_delta_2);
M_tri_C = diag(cos_delta_1Minus2-cos_delta_1Plus2);
M_tri_S = diag(sin_delta_1Minus2-sin_delta_1Plus2);
...
cvx_begin
    ...
    Igenerate = M_final*[x_optim_0;zero_set;x_optim_1;zero_set;x_optim_2;zero_set];
    minimize(beta*(norm(x_optim_0, 1)+norm(x_optim_1, 1)+norm(x_optim_2, 1))...
        + norm(Igenerate(10:390)-Y(10:390)+2, 2));
cvx_end
